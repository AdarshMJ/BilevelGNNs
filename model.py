...
class GCN(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=5,
        ...
        # C is already initialized here
        self.C = nn.Parameter(torch.Tensor(hidden_channels, hidden_channels))
    ...
    def get_representations(self, x, edge_index):
        ...
        return x  # Return the node representations before the final layer

    def forward(self, x, edge_index, p):
        # Get node representations
        node_reps = self.get_representations(x, edge_index)
        
        # Apply C to node representations
        transformed_reps = torch.matmul(node_reps, self.C)
        
        # Final layer
        out = self.convs[-1](transformed_reps, edge_index)
        return out
